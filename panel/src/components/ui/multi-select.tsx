...
const MultipleSelector = ({
    ...
    const isSingleSelectMode = maxSelected === 1;

    React.useEffect(() => {
        if (isSingleSelectMode && selected.length === 1 && open) {
            setTimeout(() => {
                setOpen(false);
                inputRef.current?.blur();
            }, 100);
        }
    }, [selected, isSingleSelectMode, open]);
    ...
    const handleKeyDown = React.useCallback(
        (e: React.KeyboardEvent<HTMLDivElement>) => {
            ...
            if (input) {
                if (e.key === 'Enter' && isSingleSelectMode && selected.length === 1) {
                    setOpen(false);
                    input.blur();
                    return;
                }
        ...
        [handleUnselect, selected, isSingleSelectMode]
    );
    ...
    useEffect(() => {
        const doSearchSync = () => {
            console.log('Calling onSearchSync with:', debouncedSearchTerm);
            const res = onSearchSync?.(debouncedSearchTerm);
            console.log('Result from onSearchSync:', res);
    ...
    const handleContainerClick = React.useCallback(() => {
        ...
        if (isSingleSelectMode && selected.length === 1) {
            return;
        }
        ...
        inputRef.current?.focus();
    }, [disabled, isSingleSelectMode, selected.length, open]);
        ...
        <Command
            ...
            <div
                className={cn(
                    ...
                    {
                        'cursor-default': isSingleSelectMode && selected.length === 1,
                        'cursor-text': !disabled && (selected.length === 0 || !isSingleSelectMode),
                    },
                    className
                )}
                onClick={handleContainerClick}>
                <div className="flex h-full items-center">
                    {isSingleSelectMode && selected.length === 1 ? (
                        <div className="flex w-full items-center justify-between px-3 py-2">
                            <span className="truncate text-sm">
                                {selected[0] && selected[0].label}
                            </span>
                            {!disabled && !selected[0]?.fixed && (
                                <button
                                    type="button"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                    ...
                                    className="text-muted-foreground hover:text-foreground ml-2 flex h-5 w-5 cursor-pointer items-center justify-center rounded-sm transition-colors"
                        ...
                        <>
                            <div className="flex flex-1 flex-wrap gap-1 px-3 py-2">
                                {selected.map((option) => (
                                    <div
                                        key={option.value}
                                        className={cn(
                            ...
                            {!hideClearAllButton && selected.length > 0 && !disabled && (
                                <button
                                    ...
                                    className="text-muted-foreground/80 hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 mr-2 flex h-6 w-6 items-center justify-center rounded-md border border-transparent transition-[color,box-shadow] outline-none focus-visible:ring-[3px]"
            ...
            {(!isSingleSelectMode || selected.length === 0) && (
